...
(function () {
  ...
  // ---- code highlighting ------------------------------------------------
  // highlight.js comes from cdnjs when the page is online; without it the
  // text renders plain. The language comes from the record's path (a tool
  // call like GET /abc.py names the file) or from the media type.
  var EXT_LANG = {
    py: "python", pyi: "python", js: "javascript", mjs: "javascript", cjs: "javascript", jsx: "javascript",
    ts: "typescript", tsx: "typescript", sh: "bash", bash: "bash", zsh: "bash", ps1: "powershell",
    c: "c", h: "c", cpp: "cpp", cc: "cpp", hpp: "cpp", cs: "csharp", rs: "rust", go: "go", java: "java",
    kt: "kotlin", swift: "swift", rb: "ruby", php: "php", lua: "lua", pl: "perl", r: "r", m: "matlab",
    json: "json", yaml: "yaml", yml: "yaml", toml: "ini", ini: "ini", cfg: "ini", conf: "ini",
    sql: "sql", html: "xml", htm: "xml", xml: "xml", svg: "xml", css: "css", scss: "scss", less: "less",
    md: "markdown", diff: "diff", patch: "diff", dockerfile: "dockerfile", makefile: "makefile",
    cmake: "cmake", nginx: "nginx", tex: "latex", vhd: "vhdl", v: "verilog", sv: "verilog", ino: "cpp"
  };
  var TYPE_LANG = {
    "application/javascript": "javascript", "text/javascript": "javascript", "application/typescript": "typescript",
    "text/x-python": "python", "application/x-python": "python", "text/x-sh": "bash", "application/x-sh": "bash",
    "text/css": "css", "text/xml": "xml", "application/xml": "xml", "image/svg+xml": "xml", "text/x-c": "c",
    "text/x-rust": "rust", "text/x-go": "go", "application/sql": "sql", "text/x-yaml": "yaml", "application/yaml": "yaml",
    "application/toml": "ini", "text/x-diff": "diff", "text/x-patch": "diff", "text/x-lua": "lua", "text/x-ruby": "ruby"
  };
  function langFor(path, type) {
    var main = (type || "").split(";")[0].trim().toLowerCase();
    if (TYPE_LANG[main]) return TYPE_LANG[main];
    var name = String(path || "").split("?")[0].split("/").pop().toLowerCase();
    if (name === "dockerfile" || name === "makefile") return EXT_LANG[name];
    var dot = name.lastIndexOf(".");
    return dot >= 0 ? EXT_LANG[name.slice(dot + 1)] || null : null;
  }
  function highlight(text, lang) {
    // Escaped HTML with highlight.js spans, or null when it cannot help.
    var hljs = window.hljs;
    if (!hljs || !lang || !hljs.getLanguage(lang) || text.length > 500000) return null;
    try { return hljs.highlight(text, { language: lang, ignoreIllegals: true }).value; } catch (e) { return null; }
  }
  function codeHtml(text, lang) {
    var inner = highlight(text, lang);
    return inner === null ? "<pre>" + esc(text) + "</pre>" : '<pre><code class="hljs">' + inner + "</code></pre>";
  }
  ...
  function renderMarkdown(md) {
    ...
    while (i < lines.length) {
      ...
      if (/^```/.test(l)) {
        para(buf); buf = [];
        var fenceLang = l.replace(/^```/, "").trim().toLowerCase(), code = []; i++;
        while (i < lines.length && !/^```/.test(lines[i])) code.push(lines[i++]);
        i++;
        var lit = highlight(code.join("\n"), EXT_LANG[fenceLang] || fenceLang);
        out.push(lit === null ? "<pre><code>" + esc(code.join("\n")) + "</code></pre>" : '<pre><code class="hljs">' + lit + "</code></pre>");
        continue;
      }
  ...
  function renderBody(bytes, type, encoding, depth, path) {
    depth = depth || 0;
    return inflate(bytes, encoding).then(function (inflated) {
      ...
      if (text) return codeHtml(t.replace(/\r\n/g, "\n"), langFor(path, main));
  ...
  function recordParts(r) {
    var bytes = typeof r.raw === "string" ? new TextEncoder().encode(r.raw) : (r.raw || new Uint8Array(0));
    var text = isText(bytes);
    var p = { row: r, bytes: bytes, text: text, headHtml: "", headers: {}, head: null, bodyBytes: bytes,
              stripped: bytes.length === 0,   // a light snapshot left the bytes out; GET /<id> has them
  ...
  function renderRecord(p, mode, pathHint) {
    if (mode === "source") {
      return Promise.resolve(p.text
        ? "<pre>" + esc(decode(p.bytes).replace(/\r\n/g, "\n")) + "</pre>"
        : "<pre>" + p.headHtml + "</pre>" + hexDump(p.bodyBytes, 65536));
    }
    // A reply has no path of its own; the caller passes the request's
    // path so a reply to GET /abc.py is highlighted as Python.
    return renderBody(p.bodyBytes, p.type, p.encoding, 0, p.row.path || pathHint || null).then(function (bodyHtml) {
  ...
  CV.langFor = langFor; CV.highlight = highlight; CV.codeHtml = codeHtml;
})();
